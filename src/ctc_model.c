...
ctc_model_test_prepares_short_input(void) {
    ...
    for (int64 i = 0; i < LENGTH(samples); i += 1) {
        ASSERT_EQUAL(input.samples[i], samples[i]);
    }
...
ctc_model_test_prepares_chunked_input(void) {
    ...
    for (int32 i = 0; i < 20; i += 1) {
        ASSERT_EQUAL(input.samples[8 + i], samples[i]);
    }
    for (int32 i = 28; i < 32; i += 1) {
        ASSERT_EQUAL(input.samples[i], 0.0f);
    }

    ASSERT_EQUAL(input.samples[32], samples[8]);
    ASSERT_EQUAL(input.samples[39], samples[15]);
    ASSERT_EQUAL(input.samples[40], samples[16]);
    ASSERT_EQUAL(input.samples[43], samples[19]);
...
ctc_model_test_chunk_metadata_three_chunks(void) {
    ...
    valid_total = 0;
    for (int64 i = 0; i < input.chunk_count; i += 1) {
        ASSERT_EQUAL(input.chunks[i].padded_start_frame, i*32);
    ...
    ASSERT_EQUAL(valid_total, input.original_sample_count);
